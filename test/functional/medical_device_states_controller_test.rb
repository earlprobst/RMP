...
#-----------------------------------------------------------------------------
#
# Filename: medical_device_states_controller_test.rb
#
#-----------------------------------------------------------------------------
...
class MedicalDeviceStatesControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    Factory(:medical_device_state, :system_state => ss, :medical_device_serial_number => md.serial_number)
    ...
    # Resources urls
    @url="/gateways/#{@gateway.serial_number}/medical_devices/#{md.serial_number}/current_state.xml"
  end
  ...
  def test_get_gateway_medical_device_current_state_xml_by_project_user
  ...
  def test_get_gateway_medical_device_current_state_xml_by_superadmin
    get(@url, nil, 'HTTP_AUTHORIZATION' => ActionController::HttpAuthentication::Basic.encode_credentials(@admin.email, @admin.password))
    assert last_response.ok?, "The response status code was not 200 OK"
  end

  def test_get_gateway_medical_device_current_state_xml_by_other_project_user
  ...
  def test_get_gateway_medical_device_current_state_xml_with_wrong_authentication
